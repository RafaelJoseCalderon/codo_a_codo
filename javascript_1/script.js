...
function ejercicio_10() {
	...
	if (cadena != null) {
		...
		for (let vocal of vocales) if (cadena.includes(vocal)) respuesta += vocal + " ";
...
function ejercicio_11() {
	...
	if (cadena != null) {
		...
		for (let caracter of cadena) if (vocales.includes(caracter)) contador++;
...
// Ejercicio 12
/* Escribe un programa que pida una frase y escriba cuántas veces aparecen cada una de
   las vocales*/
...
function ejercicio_14() {
	...
	if (!isNaN(numero)) {
		...
		for (let n of numeros) if (numero % n == 0) respuesta += n + " ";
...
function ejercicio_15() {
	...
	if (!isNaN(numero)) {
		...
		for (let i = 1; i < mitad + 1; i++) if (numero % i == 0) respuesta += i + " ";
...
// Ejercicio 16
/* Escribir un programa que escriba en pantalla los divisores comunes de dos números
   dados*/
...
/* Realizar una página con un script que calcule el valor de la letra de un número de DNI
   ...
   # Según el resultado, de 0 a 22, le corresponderá una letra de las siguientes: (T, R, W,
...
/* Un script que escriba los números del 1 al 500, que indique cuales son múltiplos de 4 y de
   9 y que cada 5 líneas muestre una línea horizontal. Por ejemplo :
   1
   2
   3
   4 (Múltiplo de 4)
...
limpiar = () => {printf(""); printf("", "enunciado")}
